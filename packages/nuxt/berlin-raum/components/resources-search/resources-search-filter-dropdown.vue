  ...
  >
    ...
    >
      ...
      >
        ...
        >
          <path d="M160 0V2H0V0H160Z" fill="black" />
        </svg>

        <span class="text-2lg font-medium px-4 py-2.5"> Zurücksetzen </span>
      </button>
    </div>
...
.reset-button {
  --gradient-overlay-height: 32px;

  &:before {
    content: '';
    position: absolute;
    left: 0;
    top: calc(var(--gradient-overlay-height) * -1);
    height: var(--gradient-overlay-height);
    background: linear-gradient(to top, var(--bg-color, white), transparent);
    width: 100%;
    pointer-events: none;
    z-index: 1;
  }
